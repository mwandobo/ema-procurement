            ...
            <div class="col-12 col-md-12 col-lg-12">
                ...
                <div class="col-lg-10">
                    ...
                    <a class="btn btn-xs btn-info" title="Collect"
                        ...
                        href="{{ route('bar_purchase.second_approval', $purchases->id)}}">First Approval</a>
                    @endif
                    @endcan
                    ...
                    <a class="btn btn-xs btn-info" title="Collect"
                        onclick="return confirm('Are you sure? you want to approve')"
                        href="{{ route('bar_purchase.final_approval', $purchases->id)}}">Final Approval</a>
                    <a class="btn btn-xs btn-danger" title="Collect"
                        onclick="return confirm('Are you sure? you want to disapprove')"
                        href="{{ route('bar_purchase.final_disapproval', $purchases->id)}}">Disapprove</a>
                    @endif
                    @endcan
                    ...
                    <a class="btn btn-xs btn-info" id="profile-tab2" data-id="{{ $purchases->id  }}" data-type="receive"
                        onclick="model({{ $purchases->id  }},'receive')" href="" data-toggle="modal"
                        data-target="#appFormModal" role="tab" aria-selected="false">Good Receive</a>

                    <a class="btn btn-xs btn-info" href="{{ route('bar_purchase.issue',$purchases->id) }}" title=""
                    ...
                    <a class="btn btn-xs btn-danger " data-placement="top"
                        href="{{ route('bar_purchase.pay',$purchases->id)}}" title="Add Payment"> Pay Purchase </a>
                    @endif


                    <a class="btn btn-xs btn-info " data-placement="top3"
                        href="{{ route('bar_purchase.get_purchase_order_tracking',$purchases->id)}}" title="Purchase Shipment Tracking"> Purchase Shipment Tracking </a>
                    ...
                    <a class="btn btn-xs btn-info" id="profile-tab2" data-id="{{ $purchases->id  }}" data-type="costing"
                        onclick="model({{ $purchases->id  }},'costing')" href="" data-toggle="modal"
                        data-target="#appFormModal" role="tab" aria-selected="false">Costing</a>

                    <a class="btn btn-xs btn-success"
                        href="{{ route('bar_issue_pdfview',['download'=>'pdf','id'=>$purchases->id]) }}" title="">
                        Download Supplier Issue </a>
                    @endif
                    <a class="btn btn-xs btn-success"
                        href="{{ route('bar_purchase_pdfview',['download'=>'pdf','id'=>$purchases->id]) }}" title="">
                        Download PDF </a>
...
?>
                            ...
                            <table class="table items invoice-items-preview" page-break-inside:="" auto;="">
                                ...
                                <tbody>
                                    ...
                                    <?php
                                         ...
                                         $due=App\Models\Bar\POS\PurchaseHistory::where('purchase_id',$purchases->id)->where('item_id',$row->item_name)->where('type', 'Purchases')->sum('quantity');
                                      $return=App\Models\Bar\POS\PurchaseHistory::where('purchase_id',$purchases->id)->where('item_id',$row->item_name)->where('type', 'Debit Note')->sum('quantity');
                                                          $qty=$due-$return;
                                         ?>
                                    <tr>
                                        <td class="">{{$i++}}</td>
                                        ...
                                        <td class="">{{ number_format($qty,2) }} {{ $row->unit}} </td>
                                ...
                                <tfoot>
                                    ...
                                    @if(!@empty($purchases->due_amount < $purchases->purchase_amount +
                                        ...
                                        <tr>
                                            <td colspan="5"></td>
                                            <td>Paid Amount</p>
                                            <td>{{number_format(( $purchases->purchase_amount +
                                                $purchases->purchase_tax) - $purchases->due_amount,2)}}
                                        ...
                                        <tr>
                                            <td colspan="5"></td>
                                            <td>Total Amount</td>
                                            <td>{{number_format($purchases->exchange_rate *
                                                ($gland_total-$purchases->discount) ,2)}} TZS</td>
                                        </tr>
                                        ...
                                        @if(!@empty($purchases->due_amount < $purchases->purchase_amount +
                                            ...
                                            <tr>
                                                ...
                                                <td>Paid Amount</p>
                                                <td>{{number_format($purchases->exchange_rate * ((
                                                    $purchases->purchase_amount + $purchases->purchase_tax) -
                                                    $purchases->due_amount),2)}} TZS</p>
                                            </tr>

                                            <tr>
                                                <td colspan="5"></td>
                                                <td class="text-danger">Total Due</td>
                                                <td>{{number_format($purchases->due_amount *
                                                    $purchases->exchange_rate,2)}} TZS</td>
                                            </tr>
                                            @endif
                                            @endif
                                </tfoot>
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0"
                                                ...
                                                style="width: 178.1094px;">Actions</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <div class="badge badge-danger badge-shadow">Waiting For First Approval
                                                </div>
                                                @elseif($row->approval_1 != '' && $row->approval_2 == '' && $row->status
                                                == 0)
                                                <div class="badge badge-danger badge-shadow">Waiting For Final Approval
                                                </div>
                                                @elseif($row->approval_1 != '' && $row->approval_2 != '' && $row->status
                                                == 0)
    ...
    @if(!empty($row->attachment))
        @php
            ...
            $icon = 'icon-download4'; // default

            if (in_array($ext, ['jpg', 'jpeg', 'png'])) $icon = 'fas fa-file-image';
            elseif ($ext === 'pdf') $icon = 'fas fa-file-pdf';
            elseif (in_array($ext, ['doc', 'docx'])) $icon = 'fas fa-file-word';

            $filePath = asset('uploads/payments/' . $row->attachment);
        @endphp

        <a href="{{ $filePath }}" target="_blank" title="Preview">
    ...
    @else
        <span><i class="fas fa-times-circle text-danger"></i> No attachment</span>
    @endif
</td>
                                            ...
                                            <td class="">
                                                <div class="form-inline">
                                                    ...
                                                    <a class="list-icons-item text-primary" title="Edit"
                                                        ...
                                                        href="{{ route('bar_purchase_payment.edit', $row->id)}}"><i
                                                            class="icon-pencil7"></i></a>&nbsp

                                                    {!! Form::open(['route' =>
                                                    ['bar_purchase_payment.destroy',$row->id],
                                                    'method' => 'delete']) !!}
                                                    {{ Form::button('<i class="icon-trash"></i>', ['type' => 'submit',
                                                    'style' => 'border:none;background: none;', 'class' =>
                                                    ...
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu">
                                                            ...
                                                            <a class="nav-link" title="Collect"
                                                                onclick="return confirm('Are you sure? you want to approve')"
                                                                href="{{ route('bar_purchase_payment.first_approval', $row->id)}}">First
                                                                Approval</a>
                                                            @endif
                                                            @endcan
                                                            ...
                                                            <a class="nav-link" title="Collect"
                                                                onclick="return confirm('Are you sure? you want to approve')"
                                                                href="{{ route('bar_purchase_payment.final_approval', $row->id)}}">Final
                                                                Approval</a>
                                                            <a class="nav-link" title="Collect"
                                                                onclick="return confirm('Are you sure? you want to disapprove')"
                                                                href="{{ route('bar_purchase_payment.first_disapproval', $row->id)}}">Disapprove</a>
                                                            @endif
                                                            @endcan
                                                            ...
                                                            <a class="nav-link" title="Collect"
                                                                ...
                                                                href="{{ route('bar_purchase_payment.confirm', $row->id)}}">Confirm
                                                                Payment</a>
                                                            @endif
                                                            @endcan

                                                            <a class="nav-link" id="profile-tab2"
                                                                href="{{ route('bar_purchase_payment_pdfview',['download'=>'pdf','id'=>$row->id]) }}"
