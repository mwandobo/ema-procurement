                    ...
                    <div class="card-body">
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <a href="{{ route('invoice.show', $invoice->id) }}">{{ $invoice->reference_no }}</a>
                                    </td>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-12">
                                @if(isset($id))
                                    {{ Form::model($id, ['route' => ['pos_invoice_payment.update', $id], 'method' => 'PUT']) }}
                                @else
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-lg-10">
                                        <input type="number" name="amount" value="{{ $invoice->due_amount ?? '' }}" class="form-control">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-lg-10">
                                        <input type="date" name="date" value="{{ $data->date ?? date('Y-m-d') }}" class="form-control" required>
                                    </div>
                                </div>
                                        ...
                                        <select class="form-control m-b" name="payment_method">
                                            ...
                                            @if(!empty($payment_method))
                                                @foreach($payment_method as $row)
                                                    <option value="{{ $row->id }}" 
                                                        @if(isset($data) && $data->manager_id == $row->id) selected @endif>
                                                        From {{ $row->name }}
                                                    </option>
                                                @endforeach
                                            @endif
                                        ...
                                        <select class="form-control m-b" name="account_id" required>
                                            ...
                                            @foreach($bank_accounts as $bank)
                                                <option value="{{ $bank->id }}" 
                                                    @if(isset($data) && $data->account_id == $bank->id) selected @endif>
                                                    {{ $bank->account_name }}
                                                </option>
                                            @endforeach
                                            @endif
                                ...
                                <div class="form-group row">
                                    <div class="col-lg-offset-2 col-lg-12">
                                        ...
                                        @else
                                            <button class="btn btn-sm btn-primary float-right m-t-n-xs" type="submit">Add Payments</button>
                                        @endif
                                    </div>
                                </div>
