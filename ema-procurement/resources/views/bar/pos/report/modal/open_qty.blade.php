  ...
  <?php $in=0; ?>
                           <table class="table datatable-basic table-striped">
                                       <thead>
                                        ...
                                        <tbody>
                                            <?php                   
                                               $account =App\Models\Bar\POS\PurchaseHistory::where('item_id', $key->id)->whereIn('location',$loc_id)->where('purchase_date','<', $start_date)->orderBy('purchase_date','desc')->get();
                                                $iaccount =App\Models\Bar\POS\GoodIssueItem::where('item_id', $key->id)->where('status',1)->whereIn('start',$loc_id)->where('date','<', $start_date)->orderBy('date','desc')->get();
                        ...
                        <?php $in+=$i->quantity; ?>
                         ...
                         <?php
                              ...
                              $q = App\Models\Bar\POS\PurchaseHistory::where('item_id', $key->id)->where('type', 'Purchases')->whereIn('location',$loc_id)->where('purchase_date','<', $start_date)->sum(\DB::raw('quantity'));
                          $r = App\Models\Bar\POS\PurchaseHistory::where('item_id', $key->id)->where('type', 'Debit Note')->whereIn('location',$loc_id)->where('purchase_date','<', $start_date)->sum(\DB::raw('quantity'));
                                            ...
                                            <tr>     
                                                <td ></td>
                                                   ...
                                                   <td></td>  <td><b> Total Balance</b></td>
                                                    <td><b>{{ number_format(($q + $in )-$r ,2) }}</b></td>
