...
<script>
 ...
 $(function() {
    let urlcontract = "{{ route('section.sales_report') }}";
        ...
        buttons: [
          {extend: 'copyHtml5',title: 'SECTION SALES REPORT  FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}} ', footer: true},
           ...
           {extend: 'csvHtml5',title: 'SECTION SALES REPORT  FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}' , footer: true},
            {extend: 'pdfHtml5',title: 'SECTION SALES REPORT  FOR THE PERIOD {{Carbon\Carbon::parse($start_date)->format('d-m-Y')}} TO {{Carbon\Carbon::parse($end_date)->format('d-m-Y')}}', footer: true},
...
$('.datatable-button-html5-basic').on('xhr.dt', function(e, settings, json, xhr ) {
  json.data = json.data.filter(function(row) {
    ...
    var a=row.qty.replace(/<\/?a[^>]*>/g, '');
     ...
     var b=row.cost.replace(/<\/?a[^>]*>/g, '');
     var purchase = parseFloat(b);
     
   
     
     if (open != '0' || purchase != '0'  ) return row
  })
})
     ...
     <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.item', function() {
                ...
                $.ajax({
                    type: 'GET',
                    url: '{{ url('reports/section/viewModal') }}',
                    data: {
                    'id': id,
                    'type': type,
